...
class ImagesCharactersDirectory {
  private $imagesCharacters = [
    [
      "name"=> "Luke Skywalker",
    ...
    public function getAll(){
      ...
      return $this->imagesCharacters;

    }

    public function setToCharacter($characters){
      ...
      for ($i= 0; $i < $size; $i++) { 
        foreach ($this->imagesCharacters as $image) {
          ...
          if($image['name'] === $characters[$i]['name']){
            $characters[$i]['image'] =  $image['image'];
            break;
          }
